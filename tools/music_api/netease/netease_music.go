...
/**
https://api.xingzhige.com/API/NetEase_CloudMusic_new/?name=%E4%B8%A4%E5%8F%AA%E8%9D%B4%E8%9D%B6%E5%BA%9E%E9%BE%99&n=1&br=4
**/
...
func getMusicInfo(name, singer string) (MusicList, error) {
	u := musicURL + fmt.Sprintf("?name=%s&br=4", name+singer)
	r, err := http.Get(u)
	if err != nil {
		return nil, err
	}
...
func getDownloadInfo(name, singer string, n int) (*MusicData, error) {
	...
	u := musicURL + fmt.Sprintf("?name=%s&n=%d&br=4", name+singer, n)
	r, err := http.Get(u)
	if err != nil {
		return nil, err
	}
